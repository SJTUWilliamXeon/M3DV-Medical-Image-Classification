...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor):
        ...
        dense1=Dense(units=1000,activation='relu')(flatten1)
        dense11=Dropout(0.3)(dense1)
        dense2=Dense(units=500,activation='relu')(dense11)
        dense22=Dropout(0.25)(dense2)
        dense3=Dense(units=100,activation='relu')(dense22)

        if num_outputs > 1:
            dense = Dense(units=num_outputs,
                          kernel_initializer="he_normal",
                          activation="softmax",
                          kernel_regularizer=l2(reg_factor))(dense3)
        else:
            dense = Dense(units=num_outputs,
                          kernel_initializer="he_normal",
                          activation="sigmoid",
